...
then
	...
	curl -sSf "https://raw.githubusercontent.com/${G_GITOWNER:=RealEmmettS}/shaughvOS/${G_GITBRANCH:=master}/shaughvos/func/shaughvos-globals" -o /tmp/shaughvos-globals || { echo 'Failed to download shaughvOS-Globals, aborting ...'; exit 1; }
...
readonly G_PROGRAM_NAME='shaughvOS-Software test'
G_CHECK_ROOT_USER "$@"
G_CHECK_ROOTFS_RW
...
image="shaughvOS_Container-$image.img"
...
Process_Software()
{
	...
	do
		...
		case $i in
			...
			103) aSERVICES[i]='shaughvos-ramlog' aCOMMANDS[i]='/boot/shaughvos/func/shaughvos-ramlog 1 && /boot/shaughvos/func/shaughvos-ramlog 0 && findmnt -t tmpfs /var/log';;
			...
			108) (( $arch == 1 )) && aCOMMANDS[i]='/mnt/shaughvos_userdata/amiberry/amiberry -h | grep '\''^\$VER: Amiberry '\' || aCOMMANDS[i]='amiberry -h | grep '\''^\$VER: Amiberry '\';;
			109) aSERVICES[i]='nfs-kernel-server' aTCP[i]='2049';;
			110) aCOMMANDS[i]='mount.nfs -V';;
			111) aSERVICES[i]='urbackupsrv' aTCP[i]='55414';;
			112) aCOMMANDS[i]='/mnt/shaughvos_userdata/dxx-rebirth/d1x-rebirth_rpigl -h';;
			...
			189) aCOMMANDS[i]='sudo -u shaughvos codium -v';;
			...
			200) aSERVICES[i]='shaughvos-dashboard-frontend shaughvos-dashboard-backend' aTCP[i]='5252 5253';;
...
then
	...
	G_EXEC rm rootfs/etc/.shaughvos_hw_model_identifier
	G_EXEC touch rootfs/boot/{bcm-rpi-dummy.dtb,config.txt,cmdline.txt}
	G_EXEC sed --follow-symlinks -i "/# Start shaughvOS-Software/iG_EXEC sed --follow-symlinks -i -e '/^G_HW_MODEL=/cG_HW_MODEL=$model' -e '/^G_HW_MODEL_NAME=/cG_HW_MODEL_NAME=\"RPi $model ($ARCH)\"' /boot/shaughvos/.hw_model" rootfs/boot/shaughvos/shaughvos-login
...
# shellcheck disable=SC2016
G_EXEC eval 'echo '\''infocmp "$TERM" > /dev/null 2>&1 || { echo "[ INFO ] Unsupported TERM=\"$TERM\", switching to TERM=\"dumb\""; export TERM=dumb; }'\'' > rootfs/etc/bashrc.d/00-shaughvos-ci.sh'

# Enable automated setup
G_CONFIG_INJECT 'AUTO_SETUP_AUTOMATED=' 'AUTO_SETUP_AUTOMATED=1' rootfs/boot/shaughvos.txt

# Apply dummy ALSA device
G_CONFIG_INJECT 'CONFIG_SOUNDCARD=' 'CONFIG_SOUNDCARD=dummy' rootfs/boot/shaughvos.txt
...
G_CONFIG_INJECT 'CONFIG_CHECK_CONNECTION_IP=' 'CONFIG_CHECK_CONNECTION_IP=127.0.0.1' rootfs/boot/shaughvos.txt
...
for i in $SOFTWARE; do G_CONFIG_INJECT "AUTO_SETUP_INSTALL_SOFTWARE_ID=$i" "AUTO_SETUP_INSTALL_SOFTWARE_ID=$i" rootfs/boot/shaughvos.txt; done

# PaperMC: Enable unattended install
if (( ${aINSTALL[181]} ))
then
	G_EXEC mkdir -p rootfs/mnt/shaughvos_userdata/papermc/plugins
	G_EXEC eval 'echo '\''eula=true'\'' > rootfs/mnt/shaughvos_userdata/papermc/eula.txt'
	G_EXEC touch rootfs/mnt/shaughvos_userdata/papermc/plugins/Geyser-Spigot.jar
fi
...
then
	...
	do
		...
		G_EXEC eval "echo -e '[Service]\nImportCredential=\nLoadCredential=' > '${i/lib/etc}.d/shaughvos-no-credentials.conf'"
	done
	...
	G_EXEC eval 'echo -e '\''[Service]\nPrivateUsers=0'\'' > rootfs/etc/systemd/system/redis-server.service.d/shaughvos-container.conf'
	G_EXEC eval 'echo -e '\''[Service]\nPrivateUsers=0'\'' > rootfs/etc/systemd/system/raspotify.service.d/shaughvos-container.conf'
	G_EXEC eval 'echo -e '\''[Service]\nPrivateUsers=0'\'' > rootfs/etc/systemd/system/navidrome.service.d/shaughvos-container.conf'
	G_EXEC eval 'echo -e '\''[Service]\nAmbientCapabilities='\'' > rootfs/etc/systemd/system/homebridge.service.d/shaughvos-container.conf'
	# Forky
	if (( $dist > 8 ))
	then
		G_EXEC mkdir rootfs/etc/systemd/system/{mariadb,systemd-logind,apache2,mpd,vaultwarden,blynkserver,gogs,whodb,lazylibrarian,bazarr,immich,immich-ml,mumble-server,shaughvos-dashboard-frontend}.service.d
		...
		G_EXEC eval 'echo -e '\''[Service]\nProtectHome=0\nProtectSystem=0'\'' > rootfs/etc/systemd/system/mariadb.service.d/shaughvos-container.conf'
		G_EXEC eval 'echo -e '\''[Service]\nProtectHome=0\nProtectSystem=0\nPrivateTmp=0\nReadWritePaths=\nProtectKernelModules=0\nProtectControlGroups=0\nProtectKernelLogs=0'\'' > rootfs/etc/systemd/system/systemd-logind.service.d/shaughvos-container.conf'
		G_EXEC eval 'echo -e '\''[Service]\nPrivateTmp=0'\'' > rootfs/etc/systemd/system/apache2.service.d/shaughvos-container.conf'
		G_EXEC eval 'echo -e '\''[Service]\nProtectSystem=0\nProtectKernelTunables=0\nProtectControlGroups=0\nProtectKernelModules=0'\'' > rootfs/etc/systemd/system/mpd.service.d/shaughvos-container.conf'
		G_EXEC eval 'echo -e '\''[Service]\nProtectHome=0\nProtectSystem=0\nPrivateTmp=0\nReadWritePaths=\nPrivateDevices=0'\'' > rootfs/etc/systemd/system/vaultwarden.service.d/shaughvos-container.conf'
		G_EXEC eval 'echo -e '\''[Service]\nPrivateTmp=0'\'' > rootfs/etc/systemd/system/blynkserver.service.d/shaughvos-container.conf'
		G_EXEC eval 'echo -e '\''[Service]\nProtectSystem=0\nPrivateTmp=0\nPrivateDevices=0\nReadWritePaths='\'' > rootfs/etc/systemd/system/gogs.service.d/shaughvos-container.conf'
		G_EXEC eval 'echo -e '\''[Service]\nProtectSystem=0\nPrivateTmp=0\nPrivateUsers=0\nProtectKernelModules=0\nProtectControlGroups=0\nProtectKernelTunables=0\nReadWritePaths='\'' > rootfs/etc/systemd/system/navidrome.service.d/shaughvos-container.conf'
		G_EXEC eval 'echo -e '\''[Service]\nProtectSystem=0\nProtectHome=0\nPrivateTmp=0\nPrivateDevices=0\nProtectKernelModules=0\nProtectControlGroups=0\nProtectKernelTunables=0\nProtectKernelLogs=0\nReadWritePaths='\'' > rootfs/etc/systemd/system/whodb.service.d/shaughvos-container.conf'
		G_EXEC eval 'echo -e '\''[Service]\nProtectSystem=0\nProtectHome=0\nPrivateTmp=0\nPrivateDevices=0\nProtectKernelModules=0\nProtectControlGroups=0\nProtectKernelTunables=0\nProtectKernelLogs=0\nReadWritePaths='\'' > rootfs/etc/systemd/system/lazylibrarian.service.d/shaughvos-container.conf'
		G_EXEC eval 'echo -e '\''[Service]\nProtectSystem=0\nProtectHome=0\nPrivateTmp=0\nPrivateDevices=0\nProtectControlGroups=0\nProtectKernelTunables=0\nReadWritePaths='\'' > rootfs/etc/systemd/system/bazarr.service.d/shaughvos-container.conf'
		G_EXEC eval 'echo -e '\''[Service]\nProtectSystem=0\nProtectHome=0\nPrivateTmp=0\nPrivateDevices=0\nProtectKernelModules=0\nProtectControlGroups=0\nProtectKernelTunables=0\nProtectKernelLogs=0\nReadWritePaths='\'' > rootfs/etc/systemd/system/immich.service.d/shaughvos-container.conf'
		G_EXEC eval 'echo -e '\''[Service]\nProtectSystem=0\nProtectHome=0\nPrivateTmp=0\nPrivateDevices=0\nProtectKernelModules=0\nProtectControlGroups=0\nProtectKernelTunables=0\nProtectKernelLogs=0\nReadWritePaths='\'' > rootfs/etc/systemd/system/immich-ml.service.d/shaughvos-container.conf'
		G_EXEC eval 'echo -e '\''[Service]\nProtectSystem=0\nProtectHome=0\nPrivateTmp=0\nPrivateDevices=0\nProtectKernelModules=0\nProtectControlGroups=0\nProtectKernelTunables=0\nProtectKernelLogs=0'\'' > rootfs/etc/systemd/system/mumble-server.service.d/shaughvos-container.conf'
		G_EXEC eval 'echo -e '\''[Service]\nProtectSystem=0\nProtectHome=0\nPrivateTmp=0\nPrivateDevices=0\nProtectKernelModules=0\nProtectControlGroups=0\nProtectKernelTunables=0\nProtectKernelLogs=0\nReadWritePaths='\'' > rootfs/etc/systemd/system/shaughvos-dashboard-frontend.service.d/shaughvos-container.conf'
		...
		G_EXEC eval 'echo -e '\''#!/bin/dash\nexec /boot/shaughvos/shaughvos-login > /dev/console 2>&1'\'' > rootfs/var/lib/shaughvos/postboot.d/shaughvos-login'
		G_EXEC sed --follow-symlinks -i '/^StandardOutput=/c\StandardOutput=journal+console' rootfs/etc/systemd/system/shaughvos-{first,post}boot.service
	fi

	# Failing 32-bit ARM Rust builds on ext4 with 64-bit host: https://github.com/rust-lang/cargo/issues/9545
	if (( $arch < 3 ))
	then
		G_EXEC eval 'echo -e '\''tmpfs /mnt/shaughvos_userdata tmpfs size=3G,noatime,lazytime\ntmpfs /root tmpfs size=3G,noatime,lazytime'\'' >> rootfs/etc/fstab'
		cat << '_EOF_' > rootfs/boot/Automation_Custom_PreScript.sh
#!/bin/dash -e
findmnt /mnt/shaughvos_userdata > /dev/null 2>&1 || exit 0
umount /mnt/shaughvos_userdata
mkdir /mnt/shaughvos_userdata_bak
mv /mnt/shaughvos_userdata/* /mnt/shaughvos_userdata_bak/
mount /mnt/shaughvos_userdata
mv /mnt/shaughvos_userdata_bak/* /mnt/shaughvos_userdata/
rm -R /mnt/shaughvos_userdata_bak
_EOF_
	fi
fi

# ARMv6: Workaround for ARMv7 Rust toolchain selected in containers with newer host/emulated ARM version
(( $arch == 1 )) && G_EXEC sed --follow-symlinks -i '/# Start shaughvOS-Software/a\sed -i '\''s/--profile minimal .*$/--profile minimal --default-host arm-unknown-linux-gnueabihf/'\'' /boot/shaughvos/shaughvos-software' rootfs/boot/shaughvos/shaughvos-login

# ARMv6/7: Workaround for "deprecated CP15 Barrier instruction" on ARMv8 host: https://github.com/RealEmmettS/shaughvOS/issues/6306#issuecomment-1515303702
...
then
	...
	G_EXEC sed --follow-symlinks -i "/# Start shaughvOS-Software/i\sed -i '/INTERFACESv4/s/\$wifi_iface/$(G_GET_NET iface)/' /boot/shaughvos/shaughvos-software" rootfs/boot/shaughvos/shaughvos-login
	G_EXEC sed --follow-symlinks -i "/# Start shaughvOS-Software/i\sed -i '/192\.168\.42\.10/! s/192\.168\.42\.1/$(G_GET_NET ip)/' /boot/shaughvos/shaughvos-software" rootfs/boot/shaughvos/shaughvos-login
	G_EXEC sed --follow-symlinks -i "/# Start shaughvOS-Software/i\sed -i 's/192\.168\.42\./$(G_GET_NET ip | sed 's/[0-9]*$//')/g' /boot/shaughvos/shaughvos-software" rootfs/boot/shaughvos/shaughvos-login
fi

# Workaround for Apache on emulated RISC-V system
if (( ${aINSTALL[83]} )) && (( $emulation && $arch == 11 ))
then
	G_EXEC sed --follow-symlinks -i '/# Start shaughvOS-Software/i\sed -i '\''/^DocumentRoot/a\Mutex posixsem'\'' /boot/shaughvos/shaughvos-software' rootfs/boot/shaughvos/shaughvos-login
fi
...
then
	G_EXEC sed --follow-symlinks -i '/# Start shaughvOS-Software/i\sed -i '\''/# Custom 1st run script/i\\rm /usr/local/bin/uname'\'' /boot/shaughvos/shaughvos-software' rootfs/boot/shaughvos/shaughvos-login
fi

# Workaround for Snapcast Client, using file output where no ALSA device is available
# shellcheck disable=SC2016
(( ${aINSTALL[192]} )) && G_EXEC sed --follow-symlinks -i '/# Start shaughvOS-Software/i\sed -i '\''s/-p \$snapcast_server_port/-p \$snapcast_server_port --player file/'\'' /boot/shaughvos/shaughvos-software' rootfs/boot/shaughvos/shaughvos-login
...
then
	...
	G_EXEC eval 'cat << '\''_EOF_'\'' > rootfs/var/lib/shaughvos/postboot.d/container.sh
#!/bin/dash
umount -l /proc/sys
...
# shellcheck disable=SC2016
# - Start all services
G_EXEC sed --follow-symlinks -i '/# Start shaughvOS-Software/a\sed -i '\''/# Custom 1st run script/a\\for i in "${aSTART_SERVICES[@]}"; do G_EXEC_NOHALT=1 G_EXEC systemctl start "$i"; done'\'' /boot/shaughvos/shaughvos-software' rootfs/boot/shaughvos/shaughvos-login
delay=10
for i in "${aDELAY[@]}"; do (( $i > $delay )) && delay=$i; done
G_EXEC eval "echo -e '#!/bin/dash\nexit_code=0; /boot/shaughvos/shaughvos-services start || exit_code=1; echo Waiting $delay seconds for service starts; sleep $delay' > rootfs/boot/Automation_Custom_Script.sh"
...
if grep -q '^aSOFTWARE_INSTALL_STATE\[$i\]=2$' /boot/shaughvos/.installed
then
_EOF_
...
G_EXEC sed --follow-symlinks -i 's|Prompt_on_Failure$|{ journalctl -n 50; ss -tulpn; df -h; free -h; systemctl start poweroff.target; exit 1; }|' rootfs/boot/shaughvos/shaughvos-login
...
G_SHAUGHVOS-NOTIFY 2 "Running container with: systemd-nspawn ${aOPTIONS[*]} -bD rootfs"
