#!/bin/bash
{
	...
	Reboot_to_load_Partition_table()
	{
		...
		ln -sfv /etc/systemd/system/shaughvos-fs_partition_resize.service /etc/systemd/system/local-fs.target.wants/shaughvos-fs_partition_resize.service
		systemctl start reboot.target
		exit 0
	}
	...
	echo '[ INFO ] Splitting output to /var/tmp/shaughvos/logs/fs_partition_resize.log'
	mkdir -pv /var/tmp/shaughvos/logs
	{
	# ---------------------------------------------------------
	echo '[ INFO ] Disabling this service to prevent possible endless loop in case of failure'
	rm -Rfv /etc/systemd/system/*.wants/shaughvos-fs_partition_resize.service
	...
	if [[ -f '/shaughvos_skip_partition_resize' ]]
	then
		rm -v /shaughvos_skip_partition_resize
	...
	then
		...
		then
			...
			for f in 'shaughvos.txt' 'shaughvos-wifi.txt' 'shaughvosEnv.txt' 'boot.ini' 'extlinux.conf' 'Automation_Custom_PreScript.sh' 'Automation_Custom_Script.sh' 'unattended_pivpn.conf' 'shaughvos-k3s.yaml'
			do
				...
				then
					...
					[[ ( $f == 'shaughvosEnv.txt' || $f == 'boot.ini' ) && -f /boot/$f ]] && mtime=$(date -r "/boot/$f" '+%s') || mtime=0
					cp -uv "$TMP_MOUNT/$f" /boot/
					[[ $f == 'shaughvosEnv.txt' || $f == 'boot.ini' ]] && (( $(date -r "/boot/$f" '+%s') > $mtime )) && REBOOT='to apply the shaughvosEnv.txt change'
				fi
			done
			umount -v "$SETUP_PART"
			rmdir -v "$TMP_MOUNT"
			unset -v TMP_MOUNT
			echo '[ INFO ] Deleting trailing shaughvOS setup partition to allow root filesystem expansion'
		...
		then
			...
			echo '[ INFO ] No shaughvOS setup partition found:'
	...
	case $ROOT_FSTYPE in
		'ext'[234])
			...
			then
				...
				resize2fs "$ROOT_DEV" || REBOOT='since the root filesystem resize failed' # https://github.com/RealEmmettS/shaughvOS/issues/6149
				...
				then
					...
					then
						...
						echo -e '#!/bin/dash\ntune2fs -c 0 '"'$ROOT_DEV' && rm /var/lib/shaughvos/postboot.d/shaughvos-reset_max_mount_count" > /var/lib/shaughvos/postboot.d/shaughvos-reset_max_mount_count
