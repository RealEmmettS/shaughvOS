#!/bin/bash
{
	...
	#
	# Info:
	# - Location: /var/lib/shaughvos/services/shaughvos-firstboot.bash
	# - Activates on first boot from shaughvos-firstboot.service, runs before shaughvos-boot.service and networking
	#////////////////////////////////////

	# Import shaughvOS-Globals --------------------------------------------------------------
	. /boot/shaughvos/func/shaughvos-globals
	readonly G_PROGRAM_NAME='shaughvOS-FirstBoot'
	G_CHECK_ROOT_USER "$@"
	G_CHECK_ROOTFS_RW
	G_INIT
	# Import shaughvOS-Globals --------------------------------------------------------------
	...
	RPi_Set_Clock_Speeds()
	{
		...
		then
			sed --follow-symlinks -i '/^#over_voltage=/c\#over_voltage=0' /boot/firmware/config.txt
			sed --follow-symlinks -i '/^#core_freq=/c\#core_freq=400' /boot/firmware/config.txt
			grep -q '^temp_limit=65$' /boot/firmware/config.txt && G_CONFIG_INJECT 'temp_limit=' 'temp_limit=75' /boot/firmware/config.txt # https://github.com/RealEmmettS/shaughvOS/issues/356
		...
		then
			...
			grep -q '^temp_limit=65$' /boot/firmware/config.txt && G_CONFIG_INJECT 'temp_limit=' 'temp_limit=75' /boot/firmware/config.txt # https://github.com/RealEmmettS/shaughvOS/issues/3019
	...
	Apply_shaughvOS_FirstRun_Settings()
	{
		# Failsafe: https://github.com/RealEmmettS/shaughvOS/issues/3646#issuecomment-653739919
		chown 0:0 /
		chmod 0755 /
		...
		then
			...
			if [[ $serial == 'VF7110A1-'* ]]
			then
				G_SHAUGHVOS-NOTIFY 2 'A revision detected, applying device tree overlay to fix Ethernet ...'
				...
				then
					G_SHAUGHVOS-NOTIFY 2 'A revision Ethernet overlay was applied already ...'
				else
			...
			if [[ $serial == *'-D008E000-'* ]]
			then
				G_SHAUGHVOS-NOTIFY 2 '8 GB RAM model detected, applying device tree overlay to make all 8 GB available to the system ...'
				read -ra overlays < <(mawk '$1=="fdtoverlays"{$1="";print}' /boot/extlinux/extlinux.conf)
				local i=
				for i in "${overlays[@]}"; do [[ $i == *'/8GB.dtbo' ]] && break; done
				if [[ $i == *'/8GB.dtbo' ]]
				then
					G_SHAUGHVOS-NOTIFY 2 '8 GB RAM overlay was applied already ...'
				else
		...
		then
			G_SHAUGHVOS-NOTIFY 2 'Running custom script, please wait ...'
			chmod +x /boot/Automation_Custom_PreScript.sh
			if /boot/Automation_Custom_PreScript.sh 2>&1 | tee /var/tmp/shaughvos/logs/shaughvos-automation_custom_prescript.log
			then
				G_SHAUGHVOS-NOTIFY 0 'Custom script'
			else
				G_SHAUGHVOS-NOTIFY 1 'Custom script: Please see the log file for more information:
         - /var/tmp/shaughvos/logs/shaughvos-automation_custom_prescript.log'
			fi
		fi
		...
		# Apply headless mode if set in shaughvos.txt (RPi, Odroid C1/C2)
		(( $G_HW_MODEL < 11 || $G_HW_MODEL == 12 )) && /boot/shaughvos/func/shaughvos-set_hardware headless "$(grep -cm1 '^[[:blank:]]*AUTO_SETUP_HEADLESS=1' /boot/shaughvos.txt)"

		# Set hostname
		/boot/shaughvos/func/change_hostname "$(sed -n '/^[[:blank:]]*AUTO_SETUP_NET_HOSTNAME=/{s/^[^=]*=//p;q}' /boot/shaughvos.txt)"

		if grep -q '^[[:blank:]]*AUTO_SETUP_AUTOMATED=1' /boot/shaughvos.txt
		then
			...
			cat << '_EOF_' > /etc/systemd/system/getty@tty1.service.d/shaughvos-autologin.conf
[Service]
ExecStart=
ExecStart=-/sbin/agetty -a root -J %I $TERM
_EOF_
			cat << '_EOF_' > /etc/systemd/system/console-getty.service.d/shaughvos-autologin.conf
[Service]
ExecStart=
...
_EOF_
		...
		local password=$(sed -n '/^[[:blank:]]*AUTO_SETUP_GLOBAL_PASSWORD=/{s/^[^=]*=//p;q}' /boot/shaughvos.txt)
		if [[ $password ]]
		then
		...
		/boot/shaughvos/func/shaughvos-set_software apt-mirror "$(sed -n "/^[[:blank:]]*$target_repo=/{s/^[^=]*=//p;q}" /boot/shaughvos.txt)"

		# Recreate machine-id: https://github.com/RealEmmettS/shaughvOS/issues/2015
		[[ -f '/etc/machine-id' ]] && rm /etc/machine-id
		[[ -f '/var/lib/dbus/machine-id' ]] && rm /var/lib/dbus/machine-id
		systemd-machine-id-setup

		# Apply time zone
		local autoinstall_timezone=$(sed -n '/^[[:blank:]]*AUTO_SETUP_TIMEZONE=/{s/^[^=]*=//p;q}' /boot/shaughvos.txt)
		...
		then
			G_SHAUGHVOS-NOTIFY 2 "Setting time zone $autoinstall_timezone. Please wait..."
		...
		local autoinstall_language=$(sed -n '/^[[:blank:]]*AUTO_SETUP_LOCALE=/{s/^[^=]*=//p;q}' /boot/shaughvos.txt)
		...
		then
			G_SHAUGHVOS-NOTIFY 2 "Setting locale $autoinstall_language. Please wait..."
			/boot/shaughvos/func/shaughvos-set_software locale "$autoinstall_language"
		fi
		...
		/boot/shaughvos/func/shaughvos-set_hardware keyboard autosetup

		# Disable serial console if set in shaughvos.txt
		grep -q '^[[:blank:]]*CONFIG_SERIAL_CONSOLE_ENABLE=0' /boot/shaughvos.txt && /boot/shaughvos/func/shaughvos-set_hardware serialconsole disable
		...
		# Apply SSH pubkey(s) from shaughvos.txt
		/boot/shaughvos/func/shaughvos-set_software add_ssh_pubkeys

		# Apply SSH password login setting
		/boot/shaughvos/func/shaughvos-set_software disable_ssh_password_logins

		# Apply forced Ethernet link speed if set in shaughvos.txt
		/boot/shaughvos/func/shaughvos-set_hardware eth-forcespeed "$(sed -n '/^[[:blank:]]*AUTO_SETUP_NET_ETH_FORCE_SPEED=/{s/^[^=]*=//p;q}' /boot/shaughvos.txt)"
		...
		# - Grab user requested settings from shaughvos.txt
		local ethernet_enabled=$(grep -cm1 '^[[:blank:]]*AUTO_SETUP_NET_ETHERNET_ENABLED=1' /boot/shaughvos.txt)
		local wifi_enabled=$(grep -cm1 '^[[:blank:]]*AUTO_SETUP_NET_WIFI_ENABLED=1' /boot/shaughvos.txt)
		local use_static=$(grep -cm1 '^[[:blank:]]*AUTO_SETUP_NET_USESTATIC=1' /boot/shaughvos.txt)
		local static_ip=$(sed -n '/^[[:blank:]]*AUTO_SETUP_NET_STATIC_IP=/{s/^[^=]*=//p;q}' /boot/shaughvos.txt)
		local static_mask=$(sed -n '/^[[:blank:]]*AUTO_SETUP_NET_STATIC_MASK=/{s/^[^=]*=//p;q}' /boot/shaughvos.txt)
		local static_gateway=$(sed -n '/^[[:blank:]]*AUTO_SETUP_NET_STATIC_GATEWAY=/{s/^[^=]*=//p;q}' /boot/shaughvos.txt)
		local static_dns=$(sed -n '/^[[:blank:]]*AUTO_SETUP_NET_STATIC_DNS=/{s/^[^=]*=//p;q}' /boot/shaughvos.txt)
		...
		then
			...
			# Apply global SSID/keys from shaughvos.txt to wpa_supplicant
			/boot/shaughvos/func/shaughvos-wifidb 1

			# Set WiFi country code
			/boot/shaughvos/func/shaughvos-set_hardware wificountrycode
		...
		then
			...
			/boot/shaughvos/func/shaughvos-set_hardware wifimodules disable

			# RPi: Keep onboard WiFi enabled until end of first run installs: https://github.com/RealEmmettS/shaughvOS/issues/5391
			(( $G_HW_MODEL > 9 )) || /boot/shaughvos/func/shaughvos-set_hardware wifimodules onboard_enable
		fi
		...
		# - IPv6
		/boot/shaughvos/func/shaughvos-set_hardware enableipv6 "$(( ! $(grep -cm1 '^[[:blank:]]*CONFIG_ENABLE_IPV6=0' /boot/shaughvos.txt) ))"
		...
		/boot/shaughvos/func/shaughvos-set_software boot_wait_for_network "$(( ! $(grep -cm1 '^[[:blank:]]*AUTO_SETUP_BOOT_WAIT_FOR_NETWORK=0' /boot/shaughvos.txt) ))"

		# Apply network time sync mirror and force sync now to speed up first run setup
		/boot/shaughvos/func/shaughvos-set_software timesync-mirror
		systemctl restart systemd-timesyncd

		# x86_64 BIOS: Set GRUB install device: https://github.com/RealEmmettS/shaughvOS/issues/4542
	...
	# Main Loop
	#/////////////////////////////////////////////////////////////////////////////////////
	# Apply shaughvos.txt settings and reset hardware ID + SSH host keys
	Apply_shaughvOS_FirstRun_Settings

	# Set install stage index to trigger automated shaughvOS-Update on login
	echo 0 > /boot/shaughvos/.install_stage

	# Disable originating service to prevent any further launch of this script
	systemctl disable shaughvos-firstboot
	#-----------------------------------------------------------------------------------
	exit 0
	#-----------------------------------------------------------------------------------
}
