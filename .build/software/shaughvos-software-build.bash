...
readonly G_PROGRAM_NAME='shaughvOS-Software build'
G_CHECK_ROOT_USER "$@"
G_CHECK_ROOTFS_RW
...
# Install Go for Gogs
[[ $NAME == 'gogs' ]] && G_CONFIG_INJECT 'AUTO_SETUP_INSTALL_SOFTWARE_ID=' 'AUTO_SETUP_INSTALL_SOFTWARE_ID=188' rootfs/boot/shaughvos.txt

# Workaround invalid TERM on login
# shellcheck disable=SC2016
G_EXEC eval 'echo '\''infocmp "$TERM" > /dev/null 2>&1 || { echo "[ WARN ] Unsupported TERM=\"$TERM\", switching to TERM=\"dumb\""; export TERM=dumb; }'\'' > rootfs/etc/bashrc.d/00-shaughvos-build.sh'
...
bash -c "\$(curl -sSf 'https://raw.githubusercontent.com/$G_GITOWNER/shaughvOS/$G_GITBRANCH/.build/software/$SCRIPT/build.bash')"${ARGS[0]:+ -- "${ARGS[@]}"}
